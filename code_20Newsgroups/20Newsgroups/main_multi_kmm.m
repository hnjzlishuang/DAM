function result = main_multi_kmm(data, C, N, alpha, kernel_types, kernel_params)
...
dv_dir = fullfile(result_dir, 'decision_values', 'multi_kmm');
if ~exist(dv_dir, 'dir')
    mkdir(dv_dir);
end
...
for kt = 1 : length(kernel_types)
    ...
    for kp = 1 : length(kernel_params{kt})
        ...
        for r = 1 : data.nRound
            ...
            dv_file = fullfile(dv_dir, sprintf('dv_round=%d_N=%d_C=%g_alpha=%g_%s_%g.mat', r, N, C, alpha, kernel_type, kernel_param));
            if exist(dv_file, 'file')
                ...
                [tmp,tmp,decision_values] = svmpredict(y(tar_index), [(1:length(tar_index))', K(tar_index,train_index)], model);
...
for r = 1 : data.nRound
    ...
    for kt = 1 : length(kernel_types)
        ...
        for kp = 1 : length(kernel_params{kt})
            ...
            log_print(result_file, '%g\t%g @ round=%d, N=%d, C=%g, alpha=%g, kernel=%s, kernel_param=%g, %s\n', ap, acc, r, N, C, alpha, kernel_type, kernel_param, data.domain_names{s});
    ...
    log_print(result_file, 'SIGMIOD %g\t%g @ round=%d, C=%g, N=%d, alpha=%g\n', ap, acc, r,N, C, alpha);
    ...
    log_print(result_file, 'NO SIGMIOD %g\t%g @ round=%d, N=%d, C=%g, alpha=%g\n', ap, acc, r, N, C, alpha);
