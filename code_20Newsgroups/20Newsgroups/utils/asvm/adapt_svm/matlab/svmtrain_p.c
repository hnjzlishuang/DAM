		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 2 && nrhs < 5)
	{
        ...
        if (param.svm_type != C_SVC && param.svm_type != ONE_CLASS && param.svm_type != C_ASVM)
        {
            mexPrintf("Error: s=%d is not supported, only C_SVC(0), ONE_CLASS(2) C_ASVM(5) are supported currently\n", param.svm_type);
            svm_destroy_param(&param);
            fake_answer(plhs);
            return;
        }
