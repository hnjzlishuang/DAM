...
//
//
// Solver for Adapt SVM Classification
//
//
class Solver_ASVM : public Solver
{
public:
    	void Solve(int l, const QMatrix& Q, const double *p_, const schar *y_,
		   double *alpha_, double Cp, double Cn, double eps,
...
protected:
 bool unshrinked;
	int select_working_set(int &out);
	int max_violating_pair(int &out_i, int &out_j);
	void do_shrinking();
	void reconstruct_gradient();
};

// reconstruct inactive elements of G from G_bar and free variables
void Solver_ASVM::reconstruct_gradient()
{	
...
// return 1 if already optimal, return 0 otherwise
int Solver_ASVM::select_working_set(int &out)
{
    double Gmin = 0;
    double Gmax = 0;
    int Gmin_idx = -1;
...
// return 1 if already optimal, return 0 otherwise
int Solver_ASVM::max_violating_pair(int &out_i, int &out_j)
{
    double Gmin = 0;
    double Gmax = 0;
    out_i = -1;
...
void Solver_ASVM::Solve(int l, const QMatrix& Q, const double *p_, const schar *y_,
		   double *alpha_, double Cp, double Cn, double eps,
		   SolutionInfo* si, int shrinking)
{
	this->l = l;
	...
	while(iter < max_iter)
	{
		...
		int i;
		if(select_working_set(i)!=0) //TODO
		{
			// reconstruct the whole gradient
			reconstruct_gradient(); //TODO
			// reset active set size and check
			active_size = l;
			info("*");
			if(select_working_set(i)!=0) //TODO
	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn)
{
	...
	Solver s;
	// In the original code of LIBSVM, prob->p is a constant term of minus_one, now prob->p is passed 
	...
	double Cp, double Cn)
{
	...
	switch(param->svm_type)
	{
		...
		case C_ASVM:
			solve_c_asvm(prob,param,alpha,&si,Cp,Cn);
			break;
	}
